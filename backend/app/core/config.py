...
class Settings(BaseSettings):
    ...
    def check_production_security(self) -> 'Settings':
        """
        Validates that critical secrets are changed in production (DEBUG=False).
        """
        if self.DEBUG:
            return self
        ...
        if self.DYNAMIC_SALT == insecure_salt:
            raise ValueError("CRITICAL SECURITY ERROR: DYNAMIC_SALT must be changed in production!")
